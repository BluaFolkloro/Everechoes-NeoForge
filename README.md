...
这是开发版本，不是正式发布。邮政网络结构可能随开发变更；升级后旧邮域绑定可能失效，需要重新为邮筒办理入网。邮筒里的信件库存会保留。完整的邮件系统仍在设计和实现中。
...
手持图册对已经加入邮区的邮筒使用，打开该邮筒所属邮区的辖区编辑界面。图册不保存邮区身份，也不是所有权或入网凭证。界面按区块坐标编辑辖区，不要求玩家到过该区块，也不加载或生成世界区块。格子只表示邮政状态（当前辖区、拟添加/移除、重叠、邮筒节点）。提交由服务端按 `DistrictCoverage` 规则与 `expectedRevision` 校验。未加入邮区的邮筒无法打开编辑界面。

### 邮筒

- `everechoes:post_box`

邮筒是一个双格方块，下半部分持有方块实体。首次右键或潜行右键打开配置：可以建立新邮区，或在邮区已登记的地图辖区内加入该邮区作为附属揽收点。邮区加入有效邮域后，右键打开 5 格交接口。多个邮筒可以共享同一邮区，但库存各自独立，信件不会自动转移。

邮域不属于玩家。内部用 UUID 标识；玩家看到的是暂定英式邮编，例如 `EV12 7QF`。`EV12` 是邮域+邮区外码，`7QF` 是未来收件端的内码。显示代码不是内部身份。
...
地图空间由邮区登记：单个邮区保持同维度连续；不同邮区可以覆盖同一区块，位置查询返回全部匹配，不按先到先得决胜。邮域范围不重复存储，而是由其有效邮区辖区的并集自动得出，邮域之间也可以重叠。空间重叠不产生所有权或自动中转。同一邮域可以包含不相连或位于不同维度的邮区。
...
- `network`：自定义网络包。
- `screen`：客户端 GUI 界面。
- `postal`：邮域/邮区、辖区、地址与 `atlasWindow` 查询。
